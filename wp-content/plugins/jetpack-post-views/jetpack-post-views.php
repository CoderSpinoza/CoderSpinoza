	...
	*/
	function get_post_views() {
		...
		if ( function_exists('stats_get_csv') ) {
			...
			$post_stats = stats_get_csv('postviews', 'days='.$random.'&limit=-1&summarize');
		...
		if ( $run_stats ) {
			...
			foreach( $allposts as $post) {
				...
				if ( array_key_exists( $post->ID, $postdata) ) {
					...
					if ( $oldViews >= 0 && $newViews > $oldViews ) {
						$this->updateMessages[ $post->ID ] = "'".get_the_title( $post->ID )."' succesfully updated with newViews(".$newViews.")";
						update_post_meta( $post->ID, 'jetpack-post-views', $newViews );
					}
					else {
						if (!$oldViews) {
							$this->updateMessages[ $post->ID ] = "'".get_the_title( $post->ID )."' failed to update because oldViews(".$oldViews.") is 0 or does not exist";
						}
						else {
							$this->updateMessages[ $post->ID ] = "'".get_the_title( $post->ID )."' did not update because newViews(".$newViews.") <= oldViews(".$oldViews.")";
						}
						add_post_meta( $post->ID, 'jetpack-post-views', ($newViews ? $newViews: 0), true );
					}
				}
				else {
					$this->updateMessages[ $post->ID ] = "'".get_the_title( $post->ID )."' failed to update because post->ID is not in postData";
					add_post_meta( $post->ID, 'jetpack-post-views', 0, true );
				}
			}
		}
	...
	function upgrade() {
		...
		$allposts = get_posts( $args );
		foreach( $allposts as $post) {
			$views = get_post_meta( $post->ID, 'jetpack-post-views', true );
...
function jetpack_post_views_on_activation() {
	...
	if ( JETPACK_POST_VIEWS_VERSION_NUM != '1.0.8' ) {
